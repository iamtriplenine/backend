...
// --- TABLE : MACHINES ACHETÉES PAR LES UTILISATEURS ---
await pool.query(`
...
// (((((((((((((((((((((((((((((((((((((((------------------------((((((((((((((((((((((((((((((((((((((((
// --- ACHAT D'UNE MACHINE ---
app.post('/invest/acheter-machine', async (req, res) => {
    ...
    try {
        ...
        // 1. Récupérer la machine
        const mRes = await client.query(
            'SELECT * FROM config_machines WHERE id_machine = $1 FOR UPDATE',
            [id_machine]
        );
        ...
        // 2. Vérifier limite globale
        const countRes = await client.query(
            'SELECT COUNT(*) FROM user_machines WHERE id_machine = $1',
            [id_machine]
        );
        if (parseInt(countRes.rows[0].count) >= machine.limite) {
            await client.query('ROLLBACK');
            return res.status(400).json({ message: "Limite globale atteinte" });
        }
        ...
        if (parseFloat(uRes.rows[0].balance) < machine.prix) {
            await client.query('ROLLBACK');
            return res.status(400).json({ message: "Solde insuffisant" });
        }
        ...
        await client.query(
            ...
            [machine.prix, id_public_user]
        );

        // 5. Créer la machine utilisateur
        await client.query(
            `INSERT INTO user_machines 
            (id_public_user, id_machine, nom_machine, gain_jour, duree_jours, date_fin, dernier_paiement)
            VALUES ($1,$2,$3,$4,$5, NOW() + ($5 || ' days')::INTERVAL, CURRENT_DATE)`,
            [
                id_public_user,
                machine.id_machine,
                machine.nom,
        ...
        await client.query('ROLLBACK');
        res.status(500).json({ message: "Erreur achat machine" });
    } finally {
        client.release();
    }
});
...
// --- CALCUL DES GAINS MACHINES ---
app.post('/invest/calculer-gains', async (req, res) => {
    try {
        ...
        for (let m of machines.rows) {
            const last = new Date(m.dernier_paiement);
            const today = new Date();

            const jours = Math.floor((today - last) / (1000 * 60 * 60 * 24));
            if (jours <= 0) continue;

            const gain = jours * parseFloat(m.gain_jour);

            await pool.query('BEGIN');
            await pool.query(
                'UPDATE utilisateurs SET balance = balance + $1 WHERE id_public = $2',
                [gain, m.id_public_user]
            );
            await pool.query(
                'UPDATE user_machines SET dernier_paiement = CURRENT_DATE WHERE id = $1',
                [m.id]
            );
            await pool.query('COMMIT');
        }

        // Désactiver les machines expirées
        await pool.query(`
        ...
        res.status(500).json({ message: "Erreur calcul gains" });
    }
});
