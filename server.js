...
const CATALOGUE_MACHINES = {
    "MINI": { prix: 500, gain: 50, jours: 20, limite: 5 },
    "STANDARD": { prix: 2000, gain: 240, jours: 15, limite: 3 },
    "PRO": { prix: 5000, gain: 700, jours: 12, limite: 2 }
};

// --- 3. ROUTE D'ACHAT MISE À JOUR ---
app.post('/invest/acheter', async (req, res) => {
    const { id_public_user, type } = req.body;
    const config = CATALOGUE_MACHINES[type];

    if (!config) return res.status(400).json({ message: "Type de machine inconnu" });
    ...
    try {
        ...
        if (parseFloat(user.rows[0].balance) < config.prix) {
            throw new Error("Solde insuffisant pour cet investissement");
        }

        // Vérifier la limite d'achat (Uniquement pour les machines encore actives)
        const nbrRes = await client.query('SELECT COUNT(*) FROM machines_achetees WHERE id_public_user = $1 AND type_machine = $2 AND jours_restants > 0', [id_public_user, type]);
        if (parseInt(nbrRes.rows[0].count) >= config.limite) {
            throw new Error(`Vous avez déjà atteint la limite de ${config.limite} pour ce modèle.`);
        }

        // Déduire le montant de l'achat
        await client.query('UPDATE utilisateurs SET balance = balance - $1 WHERE id_public = $2', [config.prix, id_public_user]);

        // Créer la machine avec ses paramètres de durée
        await client.query(
        ...
        // Historique de l'achat
        await client.query(
            `INSERT INTO transactions (id_public_user, transaction_id, montant, statut) VALUES ($1, $2, $3, $4)`,
            [id_public_user, `INV-${Date.now()}`, config.prix, `Investissement Machine ${type}`]
        );
...
// --- 4. ROUTE DE RÉCOLTE AVEC GESTION DE DURÉE ---
app.post('/invest/recolter', async (req, res) => {
    ...
    try {
        ...
        // 1. Vérifier si elle est expirée
        if (parseInt(m.jours_restants) <= 0) {
            throw new Error("Cette machine a terminé son cycle de production.");
        }

        // 2. Vérifier la date (Cycle de minuit)
        const aujourdhui = new Date().toISOString().split('T')[0];
        const derniere = new Date(m.dernier_recolte).toISOString().split('T')[0];
        if (aujourdhui === derniere) {
            throw new Error("Revenu déjà encaissé. Revenez demain après minuit.");
        }

        // 3. Procéder à la récolte
        // On crédite le solde principal
        await client.query('UPDATE utilisateurs SET balance = balance + $1 WHERE id_public = $2', [m.gain_journalier, id_public_user]);

        // On diminue les jours restants et on met à jour la date
        await client.query('UPDATE machines_achetees SET jours_restants = jours_restants - 1, dernier_recolte = CURRENT_TIMESTAMP WHERE id = $1', [machine_db_id]);

        // Historique du gain
        await client.query(
            `INSERT INTO transactions (id_public_user, transaction_id, montant, statut) VALUES ($1, $2, $3, $4)`,
            [id_public_user, `EARN-${Date.now()}`, m.gain_journalier, `Gain Journalier ${m.type_machine}`]
        );

        await client.query('COMMIT');
        res.json({ success: true, message: `+${m.gain_journalier} FCFA récoltés !` });
    } catch (e) {
        await client.query('ROLLBACK');
        res.status(400).json({ message: e.message });
    } finally { client.release(); }
